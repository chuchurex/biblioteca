"""Investigación en YouTube: buscar todos los videos sobre una persona/tema.

Uso:
    python3 scripts/investigar_youtube.py <slug> "query 1" "query 2" ...

Busca con search.list (paginado, max 4 páginas por query), deduplica por
video_id, enriquece con videos.list (duración, vistas, embeddable) y guarda
datos/investigacion_<slug>.json ordenado por vistas. Imprime resumen por canal.
"""
import json
import os
import sys
...
def inventariar_canal(channel_id, nombre):
    """Catálogo completo de un canal via su uploads playlist (UC -> UU)."""
    uploads = "UU" + channel_id[2:]
    vids, token = [], None
    while True:
        params = dict(part="snippet,contentDetails", playlistId=uploads,
                      maxResults=50)
        if token:
        ...
        for it in data.get("items", []):
            ...
            vids.append({
                ...
                "fecha": (it["contentDetails"].get("videoPublishedAt")
                          or s["publishedAt"])[:10],
                "descripcion": s.get("description", "")[:300],
            })
...
if __name__ == "__main__":
    ...
    slug, queries = sys.argv[1], sys.argv[2:]

    videos = buscar(queries)
    enriquecer(videos)

    out = os.path.join(os.path.dirname(__file__), "..", "datos",
                       f"investigacion_{slug}.json")
    ...
    for nombre, c in sorted(canales.items(), key=lambda x: -x[1]["n"])[:25]:
        print(f"  {c['n']:4d} videos | {c['vistas']:>12,} vistas "
              f"| {nombre} | {c['cid']}")
